import './pages.css'
import {Link} from 'react-router-dom';
export default function Training(){
    return (
        <body class="bodyT">
            <h1>Public & Corporate Training</h1>
            <div class="rowT">
                <h1>Public Training</h1>
            </div>
            <br></br>
            <div class="rowT">
                <h3>Leadership</h3>
            </div>
            <div class="rowT">
            <Link  to="/getcourse/Certified Agile Leadership (CAL1)">
                    <img class="imgT2" src="https://gedvillo.com/wp-content/uploads/2020/05/204f8e666371c25fc99a03470dbd6d58.png"></img>
            </Link>
            </div>
            <br></br>

            <div class="rowT">
                <h3>Scaled Agile 5.0</h3>
            </div>
            <div class="rowT">
                <Link to="/getcourse/Certified SAFe® Agilist">
                    <img class="imgT2" src = "https://images.credly.com/images/8520da4f-b343-41f7-9b0a-f032dccaa113/sa_badge.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Scrum Master">
                    <img class="imgT2" src = "https://images.credly.com/images/d9ac827f-2f5a-4017-8336-f9fab4e6881b/cert_mark_SSM_badge.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Product Owner, Product Manager">
                    <img class="imgT2" src = "https://images.credly.com/images/ad65aa49-625f-4a04-9661-0ff1b00c262c/cert_mark_POPM_badge.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Government Practitioner">
                    <img class="imgT2" src = "https://images.credly.com/images/fefcbb2c-c90c-4694-90e7-f6d61bb64a38/cert_mark_SGP_badge.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Program Consultant">
                    <img class="imgT2" src = "https://images.credly.com/images/62aa8553-6e1e-46de-a7f1-ded65844a474/cert_mark_SPC_badge.png"></img>
                </Link>
                <img class="imgT" src = "https://images.credly.com/images/db290fc4-ca36-485e-aafe-47fd33158899/twitter_thumb_201604_cert_mark_SASM_badge.png"></img>
                <Link to="/getcourse/Certified SAFe® DevOps Practitioner">
                    <img class="imgT2" src = "https://images.credly.com/images/e38c659f-8492-4869-b8da-e17c438934df/twitter_thumb_201604_cert_mark_SDP_badge.png"></img>
                </Link>
                <Link to="/getcourse/SAFe® for Architects">
                    <img class="imgT2" src = "https://images.credly.com/size/680x680/images/9a0fa926-92e5-4f20-bb02-66ff1b6f44f8/cert_mark_ARCH_badge_vector-02.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Practitioner">
                    <img class="imgT2" src = "https://images.credly.com/images/272f3108-747a-4ac9-a962-b69973a39864/cert_mark_SP_badge.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Release Train Engineer">
                    <img class="imgT2" src = "https://images.credly.com/images/69e5278d-0930-49cf-a38b-8463dd416c5d/cert_mark_RTE_badge_large_300px.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Agile Software Engineer">
                    <img class="imgT2" src = "https://images.credly.com/size/680x680/images/7b1b2279-1324-4df5-bd7b-3e558bf2a90e/cert_mark_ASE_badge_large_300px.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Lean Portfolio Manager">
                    <img class="imgT2" src = "https://images.credly.com/images/3650f17d-2601-475b-b757-d4170cf26894/cert_mark_LPM_badge_large_300px.png"></img>
                </Link>
                <Link to="/getcourse/Certified SAFe® Agile Product Manager">
                    <img class="imgT2" src = "https://images.credly.com/images/966386d9-b502-42af-a986-caf89110e87b/cert_mark_APM_badge_large_600px.png"></img>
                </Link>
            </div>
            <br></br>

            <div class="rowT">
                <h3>Scrum.org</h3>
            </div>
            <div class="rowT">
                <img class="imgT" src="https://scrumorg-website-prod.s3.amazonaws.com/drupal/inline-images/2022-09/asset_44psmi_0.png"></img>
                <Link to="/getcourse/Professional Scrum Product Owner">
                    <img class="imgT2" src="https://uploads-ssl.webflow.com/5ecfd11578751ae592c82302/63ee4a9c7a40d11298940ca1_PSPO1-professional-Product-Owner1-certificate-badge-by-scrum-org.png"></img>
                </Link>
                <Link to="Professional Scrum Developer">
                    <img class="imgT2" src="https://rmkcoaching.com/wp-content/uploads/2021/09/PSD-I-Badge-2021.png"></img>
                </Link>
            </div>
            <br></br>

            <div class="rowT">
                <h3>Scrum Alliance</h3>
            </div>
            <div class="rowT">
                
                    <img class="imgT" src="https://i0.wp.com/edinburghagile.com/wp-content/uploads/2020/08/Certified-Scrum-Master-Badge.png?fit=625%2C561&ssl=1"></img>
                
                <Link to="/getcourse/CSPO - Certified Scrum Product Owner">
                    <img class="imgT2" src="https://www.scrumalliance.org/images/default-source/badges/sa_badge_cspo.png?sfvrsn=a4355b39_1"></img>
                </Link>
                <Link to="/getcourse/CSD – Certified Scrum Developer (scrumalliance.org)">
                    <img class="imgT2" src="https://www.scrumalliance.org/images/default-source/badges/sa_badge_csd.png?sfvrsn=a94a0042_1"></img>
                </Link>
            </div>
            <br></br>

            <div class="rowT">
                <h3>PMI</h3>
            </div> 
            <div class="rowTF"> 
                <Link to="/getcourse/Project Management Professional">
                    <img class="imgT2" src="https://images.credly.com/images/260e36dc-d100-45c3-852f-9d8063fa71e6/pmp-600px.png"></img>
                </Link>
                <Link to="/getcourse/PMI Agile Certified Practitioner (PMI-ACP)">
                    <img class="imgT2" src="https://images.credly.com/images/60de4761-84eb-4d9f-a6bc-c290c2e65985/pmi-acp-600px.png"></img>
                </Link>
            </div>

            <div class="rowTF">
                <h1>Corporate Training</h1>
            </div>
            <div class="rowT">
                <Link to="/getcourse/Value Stream Corporate Workshop">
                <div class="tbutton">
                    <button class="btnt"><b>Value Stream <br></br> Workshop</b></button>
                </div>
                </Link>
                <Link to="/getcourse/SAFe® Portfolio Management">
                <div class="tbutton">
                    <button class="btnt"><b>SAFe® Portfolio<br></br> Management</b></button>
                </div>
                </Link>
                <Link to="/getcourse/Program Increment Workshop (customized offering)">
                <div class="tbutton">
                    <button class="btnt"><b> Program Increment<br></br> Workshop</b></button>
                </div>
                </Link>
                <Link to="/getcourse/Inspect & Adapt (customized offering)">
                <div class="tbutton">
                    <button class="btnt"><b>Inspect & <br></br> Adapt</b></button>
                </div>
                </Link>
                <Link to="/getcourse/Product Management Workshop (customized offering)">
                <div class="tbutton">
                    <button class="btnt"><b>Product Management<br></br> Workshop</b></button>
                </div>
                </Link>
                <Link to="/getcourse/Team Building Workshop">
                <div class="tbutton">
                    <button class="btnt"><b>Team <br></br>Building</b></button>
                </div>
                </Link>
                <Link to="/getcourse/Customized Scrum and Kanban Workshop">
                <div class="tbutton">
                    <button class="btnt"><b> Customized <br></br>Scrum/Kanban<br></br>Workshop</b></button>
                </div>
                </Link>

            </div>


    
        </body>
        )

}